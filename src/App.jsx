...
import { import React, { useState, useEffect, useMemo, useRef } from 'react';
import { initializeApp } from 'firebase/app';
import { 
  getAuth, 
  signInAnonymously, 
  ...
  if (!user) return (
    <div className="flex min-h-screen items-center justify-center bg-slate-100 p-4">
      <div className="bg-white p-8 rounded-2xl shadow-xl max-w-md w-full text-center animate-in fade-in zoom-in duration-300">
        ...
        <Button onClick={async () => { try { await signInWithPopup(auth, googleProvider); } catch(e) { alert(e.message); } }} variant="google" className="w-full flex items-center justify-center gap-3 py-3 text-lg shadow-sm">
        ...
        <div className="p-6 hidden md:block">
          ...
          <div className="flex items-center gap-2 mt-3 p-2 bg-slate-800 rounded-lg">
            ...
            <div className="overflow-hidden">
              ...
              <p className="text-[10px] text-slate-500">v19 Stats Pro</p>
            </div>
          </div>
        </div>
...
const TimerView = ({ subjects, exams, userId }) => {
  ...
  const bestSubject = useMemo(() => {
    ...
    subjects.forEach(s => {
      ...
      if(minDays <= 1) score *= 100;
      else if(minDays <= 3) score *= 50;
      else if(minDays <= 7) score *= 10;
          ...
          {bestSubject && (
            <div className="bg-amber-50 border border-amber-200 rounded-lg p-4 text-left flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-amber-700 mt-1">Basado en tus fechas de examen y dificultad.</p>
...
const StatsView = ({ subjects, history }) => {
  const stats = useMemo(() => {
    ...
    const subjectTotals = {};
    history.forEach(h => {
      subjectTotals[h.subject] = (subjectTotals[h.subject] || 0) + h.minutes;
    });
    const topSubjectName = Object.keys(subjectTotals).reduce((a, b) => subjectTotals[a] > subjectTotals[b] ? a : b, "");
    ...
    const weeklyData = last7Days.map(date => {
        ...
        return { date, minutes: dayMins };
    });

    const maxDayMins = Math.max(...weeklyData.map(d => d.minutes), 1);

    // 5. Subject Breakdown
    const breakdown = Object.entries(subjectTotals)
        .sort(([,a], [,b]) => b - a)
        .map(([name, minutes]) => ({ 
            name, 
            minutes, 
            percentage: Math.round((minutes / totalMinutes) * 100),
            sessions: history.filter(h => h.subject === name).length
        }));

    return { totalMinutes, topSubjectName, streak, weeklyData, maxDayMins, breakdown };
  }, [history]);

  if (!stats) return (
    <div className="flex flex-col items-center justify-center h-full text-slate-400 animate-in fade-in">
        <BarChart3 size={64} className="mb-4 opacity-20"/>
        <p>Completa tu primera sesión de estudio para ver estadísticas.</p>
    </div>
  );

  return (
    <div className="space-y-6 animate-in fade-in pt-4 pb-8">
      {/* KPI Cards */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-white p-5 rounded-xl shadow-sm border border-slate-100 flex items-center gap-4">
          <div className="p-3 bg-blue-50 text-blue-600 rounded-lg"><Clock size={24}/></div>
          <div>
            <p className="text-slate-500 text-xs font-bold uppercase">Tiempo Total</p>
            <p className="text-2xl font-bold text-slate-800">{(stats.totalMinutes/60).toFixed(1)}h</p>
          </div>
        </div>
        <div className="bg-white p-5 rounded-xl shadow-sm border border-slate-100 flex items-center gap-4">
          <div className="p-3 bg-orange-50 text-orange-600 rounded-lg"><Flame size={24}/></div>
          <div>
            <p className="text-slate-500 text-xs font-bold uppercase">Racha Actual</p>
            <p className="text-2xl font-bold text-slate-800">{stats.streak} días</p>
          </div>
        </div>
        <div className="bg-white p-5 rounded-xl shadow-sm border border-slate-100 flex items-center gap-4">
          <div className="p-3 bg-purple-50 text-purple-600 rounded-lg"><Trophy size={24}/></div>
          <div>
            <p className="text-slate-500 text-xs font-bold uppercase">Asignatura Top</p>
            <p className="text-xl font-bold text-slate-800 truncate max-w-[120px]">{stats.topSubjectName || "-"}</p>
          </div>
        </div>
      </div>
      ...
      <Card className="h-64 flex flex-col">
        <div className="flex justify-between items-center mb-6">
          <h3 className="font-bold text-slate-700 flex items-center gap-2"><TrendingUp size={18}/> Actividad Últimos 7 Días</h3>
        </div>
          ...
          {stats.weeklyData.map((day, i) => (
            <div key={i} className="flex-1 flex flex-col items-center group">
               <div className="relative w-full flex justify-center">
                   <div 
                     className="w-full max-w-[30px] bg-blue-500 rounded-t-md hover:bg-blue-600 transition-all duration-500 min-h-[4px]" 
                     style={{ height: `${(day.minutes / stats.maxDayMins) * 150}px` }}
                   ></div>
                   {/* Tooltip */}
      ...
      {/* Subject Breakdown */}
      <Card>
        <h3 className="font-bold text-slate-700 mb-4 flex items-center gap-2"><ListTodo size={18}/> Desglose Detallado</h3>
            ...
            <div key={item.name} className="group">
              <div className="flex justify-between text-sm mb-1">
                ...
                <div className="flex gap-3 text-slate-500 text-xs">
                    <span className="flex items-center gap-1"><Clock size={12}/> {item.minutes}m</span>
                    <span className="flex items-center gap-1"><CalendarDays size={12}/> {item.sessions} ses.</span>
                </div>
              </div>
              <div className="w-full h-2 bg-slate-100 rounded-full overflow-hidden">
                <div 
                  className="h-full bg-indigo-500 group-hover:bg-indigo-600 transition-all duration-500" 
      ...
      <nav className="md:w-64 bg-slate-900 text-slate-300 flex md:flex-col justify-between md:h-screen z-50 order-2 md:order-1 shrink-0 shadow-xl">
        <div className="p-6 hidden md:block">
          ...
          <div className="flex items-center gap-2 mt-3 p-2 bg-slate-800 rounded-lg">
            ...
            <div className="overflow-hidden">
              <p className="text-xs text-slate-300 font-medium truncate max-w-[120px]">{user.isAnonymous ? 'Invitado' : user.displayName}</p>
              <p className="text-[10px] text-slate-500">v18.2 Progress Bar</p>
            </div>
          </div>
        </div>
...
// 2. TO-DO VIEW (CON BARRA DE PROGRESO)
const TodoView = ({ subjects, exams, todos, userId }) => {
  const [hours, setHours] = useState(() => localStorage.getItem('sm_hours') || 2);
  const [sel, setSel] = useState(() => {
    try {
  ...
  const gen = async () => {
    ...
    finalList.forEach(name => {
      ...
      // Factor de Urgencia
      if (minDays === 0) base *= 50;      
      else if (minDays <= 1) base *= 30;  
      else if (minDays <= 2) base *= 15;   
      else if (minDays <= 7) base *= 8;   
      else if (minDays <= 14) base *= 3;  
      else base *= 1.5; 
